...
/*
        class Animation{
            public:
                ////////////////////////////////////////////////////////////////////////////
                // Это можно вывести в отдельный класс TextureRegion //
                // Который раскладывает текстурный атлас на Frames //
                int columns;           //столбцы
                int rows;                //ряды
                                              //массив фреймов [columns][rows]
                Texture texture;    //ссылка на текстуру
                // Это можно вывести в отдельный класс TextureRegion //
                ////////////////////////////////////////////////////////////////////////////
                float speed;          //скорость проигрывания анимации
                                             //список анимаций
                //одна анимация - это массив [column][row]
                //
                //если в массиве встречается -1, то при проходке по массиву
                //анимации, этот кадр упускается.  Т.о. можно гибко настроить
                //анимацию.
                //
                //
               // Для анимаций как в RPG Maker используется [direction][state]
                //
        };

        class AnimationRPG : public Animation  {
            AnimationRPG(Texture t, float s){
                this->texture = t;
                this->speed = s;
                this->columns = 4;
                this->rows = 4;
...
*/
        ...
        class Texture      /*Texture*/
        {
            public:
                class Shader
                {
                    ...
                    public:
                        ...
                        Shader(){
                            ...
                            "void main()\n"
                            "{\n"
                            "   gl_Position = MVP * vec4(position.x, position.y, z, 1.0f);\n"
                            "   vertexColor = color;\n"
                ...
                class Mesh
                {
                    ...
                    public:
...

                        {
                            ...
                            //это не для прозрачных
                            //glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
                            //glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
                            //это для прозрачных для избегания бага появления рамок текстуры
                ...
                float z; //zBuffer
                float alpha;
                bool init;  //проверка загрузки изо

                //затем убрать это
                glm::mat4 MVP;

                Texture(const char *p, float zBuffer, float a){
                    this->z = zBuffer;
                    this->path = p;
                    this->alpha = a;
                    this->init = false;
                    ...
                    int channels;
                    unsigned char *image = stbi_load(this->path,&this->width,&this->height,&channels, STBI_rgb_alpha);
                    this->init = true;
                    if(image == NULL) {
                    ...
                    this->mesh = new Mesh(&this->shader->shaderProgram, image, this->width, this->height);
                ...
                void draw(){
                    ...
                    glUniform1f(alphaLoc, this->alpha);

                    GLuint MVPLoc = glGetUniformLocation(this->shader->shaderProgram, "MVP");
                    glUniformMatrix4fv(MVPLoc, 1, GL_FALSE, glm::value_ptr(MVP));

                    GLuint zLoc = glGetUniformLocation(this->shader->shaderProgram, "z");
                    glUniform1f(zLoc, this->z);

                    GLuint texLoc = glGetUniformLocation(this->shader->shaderProgram, "textureMatrix");
                    glUniformMatrix4fv(texLoc, 1, GL_FALSE, glm::value_ptr(glm::mat4(1.0f)));
                ...
                void drawRegion(float srcX, float srcY, float srcWidth, float srcHeight){
                    ...
                    //glm::vec3 texPosition = glm::vec3(srcX / this->width, srcY / this->height, 0.0f); //Это для пикселей
                    glm::vec3 texPosition = glm::vec3(srcX, srcY, 0.0f); //это для процентов
                    texMatrix = glm::translate(texMatrix, texPosition);
                    //glm::mat4 texMatrix = m4.translation(srcX / this->width, srcY / this->height, 0);

                    //glm::vec3 texScale    = glm::vec3(srcWidth / this->width, srcHeight / this->height, 1.0f); //Это для пикселей
                    glm::vec3 texScale    = glm::vec3(srcWidth, srcHeight, 1.0f); //это для процентов
                    texMatrix = glm::scale(texMatrix, texScale);
                    ...
                    GLuint texLoc = glGetUniformLocation(this->shader->shaderProgram, "textureMatrix");
                    glUniformMatrix4fv(texLoc, 1, GL_FALSE, glm::value_ptr(texMatrix));
        ...
        void setScreen(Screen *s){
            ...
            // Инициализируем GLEW здесь, иначе ошибка
            GLenum err = glewInit();
            if (GLEW_OK != err){
        ...
        Viewport *createViewport(int w,int h){
            return new Viewport(w,h);
        }

        Screen *createScreen(Viewport *v){
            return new Screen(v);
        }
        ...
        Texture *createTexture(const char *p, float zBuffer, float a){
